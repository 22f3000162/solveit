...
import { Faker, en } from "@faker-js/faker"; // Correct ES Module import
import { format } from "prettier";
import prettierMarkdown from "prettier/plugins/markdown.js";
import { chromium } from 'playwright-chromium'; // Import Playwright
...
// Corrected tableGenerator function (remains the same as last fix)
function tableGenerator(seed, rows, cols) {
    ...
    for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
            let v = Math.floor(rng() * 100);
            if (v > 90) { row.push(v - 10); }
            else if (v < 10) { row.push(v + 10); }
            else { row.push(v); }
        }
        data.push(row);
    }
...
// *** Q10 FIX: Using the EXACT function from the hint ***
function generateFakeMarkdownRt(seedNumber) {
    // Create a new Faker instance WITH the seed
    const fakerInstance = new Faker({
        locale: [en], // Use the imported English locale
        seed: seedNumber
    });

    let markdownOutput = "";
    const elementTypes = ["header", "list", "paragraph", "code", "table", "blockquote", "link"];
    // Use fakerInstance for all calls
    const numElements = fakerInstance.number.int({ min: 10, max: 30 });

    for (let i = 0; i < numElements; i++) {
        const choice = fakerInstance.helpers.arrayElement(elementTypes);
        switch (choice) {
            case "header": {
                const level = fakerInstance.number.int({ min: 1, max: 6 });
                // v9+ uses min/max directly in sentence/words
                const sentence = fakerInstance.lorem.sentence({ min: 2, max: 6 }).slice(0, -1);
                markdownOutput += `${"#".repeat(level)} ${sentence}\n\n`;
                break;
            }
            case "list": {
                const listChar = fakerInstance.helpers.arrayElement(["-", "*", "+"]);
                const numItems = fakerInstance.number.int({ min: 2, max: 5 });
                for (let j = 0; j < numItems; j++) {
                    const words = fakerInstance.lorem.words({ min: 2, max: 5 }); // Pass object for count
                    markdownOutput += `${listChar} ${words}\n`;
                }
                markdownOutput += "\n";
                break;
            }
            case "paragraph":
                markdownOutput += `${fakerInstance.lorem.paragraph()}\n\n`;
                break;
            case "code": {
                const lang = fakerInstance.helpers.arrayElement(["javascript", "python", "bash", ""]);
                 // v9+ uses min/max directly
                const lines = fakerInstance.lorem.lines({ min: 1, max: 5 });
                markdownOutput += "```" + lang + "\n" + lines + "\n```\n\n";
                break;
            }
            case "table": {
                const numCols = fakerInstance.number.int({ min: 2, max: 5 });
                const numRows = fakerInstance.number.int({ min: 2, max: 5 });
                const header = "| " + fakerInstance.lorem.words(numCols).split(" ").join(" | ") + " |\n"; // Count is direct arg
                const separator = "|" + ":---|".repeat(numCols) + "\n";
                let rows = "";
                for (let j = 0; j < numRows; j++) {
                    rows += "| " + fakerInstance.lorem.words(numCols).split(" ").join(" | ") + " |\n"; // Count is direct arg
                }
                markdownOutput += header + separator + rows + "\n";
                break;
            }
            case "blockquote":
                markdownOutput += `> ${fakerInstance.lorem.sentence()}\n\n`;
                break;
            case "link":
                 // v9+ uses count object
                markdownOutput += `[${fakerInstance.lorem.words({ min: 2, max: 2 })}](${fakerInstance.internet.url()})\n\n`;
                break;
        }
    }
...
// --- Question 1-9 & 11-12 (Remain the same as previous correct version) ---
...
async def get_outline(country: str):
    ...
    try:
        ...
        # Return as plain text (adjust content type if needed by validator)
...
async function solveQ5_Nominatim({ user }) {
    ...
    try {
        ...
        if (!p || p.length === 0) throw new Error(`No city results found for ${o}, ${s}`);
        if (!p[0].boundingbox || p[0].boundingbox.length !== 4) throw new Error(`Invalid boundingbox data for ${o}, ${s}`);
...
async function solveQ6_HackerNews({ user }) {
    ...
    try {
        ...
        const f = await fetch(PROXY_URL(url)).then(l => {
            if (!l.ok) throw new Error(`HN RSS Fetch failed: ${l.status} ${l.statusText}`);
            return l.text();
        });
        const dom = new JSDOM(f, { contentType: "text/xml" });
        const g = dom.window.document.querySelector("item link");
        if (!g) throw new Error(`No items found for topic "${o}" with >= ${s} points.`);
        u = g.textContent;
    } catch (err) {
        error = `Failed to fetch or parse data. Error: ${err.message}`;
    }
...
async function solveQ7_GitHubUser({ user }) {
    ...
    try {
        ...
        const l = await fetch(searchUrl, {
             headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'Node.js-Solver-App' }
        }).then(p => p.ok ? p.json() : Promise.reject(`GitHub Search API error: ${p.status} ${p.statusText}`));

        if (!l.items || l.items.length === 0) throw new Error(`No users found for location:${o} followers:>=${s}`);

        const userUrl = l.items[0].url;
         const userData = await fetch(userUrl, {
              headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'Node.js-Solver-App' }
         }).then(p => p.ok ? p.json() : Promise.reject(`GitHub User API error: ${p.status} ${p.statusText}`));

        c = userData.created_at;
    } catch (err) {
         console.warn(`Direct GitHub API call failed for Q7: ${err.message}. Retrying with proxy...`);
         try {
             const searchUrl = `https://api.github.com/search/users?q=location:${o}+followers:>=${s}&sort=joined&order=desc`;
             const l = await fetch(PROXY_URL(searchUrl)).then(p => p.json());
             if (!l.items || l.items.length === 0) throw new Error(`No users found (proxy) for location:${o} followers:>=${s}`);
             const userUrl = l.items[0].url;
             const userData = await fetch(PROXY_URL(userUrl)).then(p => p.json());
             c = userData.created_at;
         } catch (proxyErr) {
            error = `Failed to fetch data (direct & proxy). Error: ${proxyErr.message}. You might be rate-limited by GitHub.`;
         }
    }
...
async function solveQ10_PDFMarkdown({ user }) {
    ...
    try {
        ...
        // 2. Generate the unformatted markdown content from that seed using the correct Faker v9 logic
        const c = generateFakeMarkdownRt(seed); // Use the hint's exact function
        // 3. Format it with Prettier v3.4.2 to get the correct answer
...
async function solveQ12_HTMLTable({ user }) {
    ...
    try {
        ...
        if (!table) throw new Error(`No table found on page ${n}.`);
...
// *** Q13 FIX: Using actual Playwright ***
async function solveQ13_Playwright({ user }) {
    ...
    const n = Math.floor(r() * 90); // start seed number (0-89)
    // Array of 10 seed STRINGS: n, n+1, ..., n+9
    const o = Array.from({ length: 10 }, (d, l) => (n + l).toString());

    let grandTotal = 0;
    let error = null;
    const log = [];
    let browser = null;
    let question = `Question: What is the total sum of all numbers scraped from tables for seeds ${o[0]} to ${o[o.length-1]}?`;
    ...
    try {
        ...
        try {
            // Try launching normally first
             ...
             log.push(`Initial browser launch failed: ${launchError.message}. Ensure Playwright browsers are installed ('npx playwright install chromium --with-deps').`);
             // You might add automatic install here if needed, but it's often better manual
             throw new Error(`Failed to launch browser. Please run 'npx playwright install chromium --with-deps' manually.`);
        }

        const context = await browser.newContext({ userAgent: 'Mozilla/5.0' }); // Use a common user agent
        const page = await context.newPage();
        log.push(`Scraping ${o.length} seeds starting from ${n}...`);

        for (const currentSeed of o) {
            const url = `https://sanand0.github.io/tdsdata/js_table/?seed=${currentSeed}`;
            log.push(`  Navigating to seed=${currentSeed}...`);
            let pageTotal = 0;
            try {
                await page.goto(url, { waitUntil: 'networkidle0', timeout: 30000 }); // Wait longer for network idle

                // Wait for at least one table cell to ensure data is loaded
                 await page.waitForSelector('table td', { timeout: 20000 });

                // Use page.evaluate to perform DOM manipulation in the browser context
                pageTotal = await page.evaluate(() => {
                    ...
                    tables.forEach(table => {
                        ...
                        cells.forEach(cell => {
                            const num = parseInt(cell.innerText, 10);
                            if (!isNaN(num)) {
                                sum += num;
                            }
                        });
                ...
                log.push(`  Sum for seed ${currentSeed} is ${pageTotal}`);
                grandTotal += pageTotal;

            } catch (e) {
                log.push(`  Warning: Error processing seed ${currentSeed}: ${e.message}. Skipping this page.`);
                // Continue to the next seed instead of failing the whole task
            }
        ...
        error = `Playwright Task Failed: ${err.message}. Ensure browsers are installed ('npx playwright install chromium --with-deps'). Log: ${log.join('; ')}`;
        if (browser) {
            await browser.close();
        }
    }
    ...
    const answerPayload = {
         title: a,
         question,
         answer: error ? error : grandTotal.toString(),
         log: log // Add the log array here
    };
...
app.post("/solve", async (req, res) => {
    ...
    const solvers = [
        ...
        solveQ11_Crawl,
        solveQ12_HTMLTable,
        solveQ13_Playwright // Using the new Playwright version
    ];

    try {
        // Run all solvers in parallel
        const results = await Promise.all(solvers.map(solver => solver({ user })));
        console.log(`Successfully solved questions for ${email}`);
        // Send the results back to the frontend
        res.json(results);

    } catch (error) {
        console.error("An error occurred while solving:", error);
        res.status(500).json({ error: "An internal server error occurred during solving." });
    }
});
